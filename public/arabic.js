...
(function() {
	...
	function homepage () {
		...
		hide();
		let response  = fetching("http://localhost:process.env.PORT/?mode=all&title=");
	...
	*/
	function comments(folder) {
		...
		section.innerHTML = "";
		let response  = fetching("http://localhost:process.env.PORT/?mode=info&title="+folder);
		response.then(function(responseText) {
			response  = JSON.parse(responseText);
	...
	*/
	function description(folder) {
		let response  = fetching("http://localhost:process.env.PORT/?mode=description&title="+folder);
		response.then(function(responseText) {
			response  = JSON.parse(responseText);
	...
	function post(){
		...
		if (text) {
			...
			fetch("http://localhost:process.env.PORT/?mode=post&title="+num, fetchOptions)
				.then(checkStatus)
